...
bool ensureSPIFFS() {
  ...
  // First attempt: mount without formatting
  if (SPIFFS.begin(false)) {
    spiffsMounted = true;
  } else {
    Serial.println("⚠️ SPIFFS mount failed, attempting format...");

    // Second attempt: format + mount (fixes brand new ESPs)
...
void setup() {
  ...
  if (!ensureSPIFFS()) {
    Serial.println("❌ SPIFFS unavailable at boot (partition or flash issue)");
  } else {
